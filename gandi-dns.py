...
def get_local_ip(opts):
    if opts.interface is None:
        ...
        try:
            ...
            print('[error] interface {} not found'.format(opts.interface),
                  file=sys.stderr)
            sys.exit(1)
...
def change_zone_ip(new_ip, opts):
    ...
    zone_record ={
        'name': opts.a_name, 'value': new_ip, 'ttl':opts.ttl, 'type': 'A'}

    new_zone_ver = api.domain.zone.version.new(
        opts.production_key, get_zone_id(opts))

    # clear old A record (defaults to previous verison's
    api.domain.zone.record.delete(
        opts.production_key, get_zone_id(opts),
        new_zone_ver, {'type':'A', 'name': opts.a_name}
    )
    ...
    api.domain.zone.version.set(
        opts.production_key, get_zone_id(opts), new_zone_ver)
...
def main():
    ...
    if (zone_ip.strip() == current_ip.strip()):
        ...
        print('[info] DNS Mistmatch detected: A-record: ',
